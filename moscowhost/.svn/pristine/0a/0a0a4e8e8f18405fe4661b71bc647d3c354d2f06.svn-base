...
class Menu(object):
    ...
    def add(self, reverse_conf, verbose_name, attrs = None, isCallUs = False):
        """
        ...
        if not isCallUs:
            self.items.append({
                ...
                'isCallUs' : False # !!!!!!! это дурацкий выход, при переверстке сайта надо будет придумать что-то другое, более общее
            })
        else:
            self.items.append({
                'name': reverse_conf.view_name, # url name from urls.py
                ...
                'is_current': self.check_current(reverse_conf),
                'attrs': self.flatten_attrs(attrs),
                'isCallUs' : True # !!!!!!! это дурацкий выход, при переверстке сайта надо будет придумать что-то другое, более общее
            })
